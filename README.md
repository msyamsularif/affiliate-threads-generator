...
| 6   | First-hand claims only with a stored testimony | The row's `Used`/`Testimonial` answer picks the mode (`none` / `firsthand`); `threads_publish` derives it from the Sheet and refuses what the testimony does not support                |
| 7   | Failed publish never changes Status            | Sheet write happens only after a confirmed media ID                                                                                                                                     |
| 8   | AI cannot reach Threads any other way          | No credential or publish path exists outside the tool                                                                                                                                   |
| 9   | Shopee anti-bot is never bypassed              | Skill instructions; research is review-first and never depends on the seller's page                                                                                                     |
| 10  | No secret ships in the repository              | Declared as names only (`requires_env` / `optional_env` + skill frontmatter); Hermes prompts for them at install and owns the values — see [`docs/credentials.md`](docs/credentials.md) |
| 11  | Reach comes from the topic tag, not hashtags   | `threads_publish` rejects stray hashtags in the copy and requires one topic tag, validated against the platform's limits                                                                |
| 12  | Guarantee language never ships                 | `amplifier_language` blocks "dijamin", "100% ampuh", "pasti sembuh" in both experience modes                                                                                            |
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `test_guardrails.py`             | Character/link limits, the affiliate URL, hashtags, topic tags, the fabricated-experience ban                                 |
| `test_experience.py`             | The row's experience mode, the provenance guardrails, and `set_experience.py` — including that the lint and the publish agree |
