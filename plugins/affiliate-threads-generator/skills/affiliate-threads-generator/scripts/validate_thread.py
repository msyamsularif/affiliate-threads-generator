...
def main(argv: list[str] | None = None) -> int:
    ...
    if product_id and not (affiliate_url and product_name):
        try:
    ...
    report = guardrails.validate_thread(
        posts, settings, affiliate_url=affiliate_url, product_name=product_name
    )
    ...
    payload = {
        ...
        "limits": {
            "max_chars_per_post": settings.max_chars_per_post,
            "max_links_per_post": settings.max_links_per_post,
...
def _print_text(payload: dict) -> None:
    print(f"posts: {payload['posts']}  chars: {payload['char_counts']}  links: {payload['link_counts']}")
